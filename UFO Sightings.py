...
sorted_duration = sorted(seconds_float, key = lambda x: float(x["duration (seconds)"]))
for row in sorted_duration:
    print(row['datetime'], row['state'], row["duration (seconds)"])
...
count = {shape:sightings_shapes.count(shape) for shape in sightings_shapes}
print(count)
